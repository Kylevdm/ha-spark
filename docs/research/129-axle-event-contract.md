...
- **Verified first-party:** the HA fallback endpoint's shape, auth, polling
  hint, and its four fields. It gives **absolute ISO-8601 start/end times (with
  the supplied offset) and an explicit `"import"`/`"export"` direction** —
  enough to schedule a window and
...
- **Verified first-party:** the VPP dispatch webhook's full event contract
  (identity, UTC window, signed power target in kW, retry semantics). It is the
  only surface that carries a power target, and it is not available to a household on the
  documented path.
...
1. Axle developer docs (Mintlify), including the per-page `.md` sources and the
   ...
   - Webhooks (envelope, signature, retries): <https://docs.axle.energy/workflows/webhooks.md>
...
4. Community HACS integration `deanhalllincoln/ha-axle-vpp`, tag `v1.0.13`
   (the inspected manifest reports internal version `1.0.12`), read at tag:
...
([webhooks](https://docs.axle.energy/workflows/webhooks.md), [dispatch](https://docs.axle.energy/workflows/axle-vpp/api-reference/dispatch)).
...
Delivery contract ([webhooks](https://docs.axle.energy/workflows/webhooks.md)):
...
| --- | --- | --- |
| Absolute start/end | ✅ ISO-8601 `start_time`/`end_time` (offset semantics not documented) | ✅ UTC `start_time`/`end_time` |
| Direction (export vs import) | ✅ `import_export == "export"` | ✅ `power_kw < 0` |
| Payment/export rate | ❌ none — must be configured/assumed | ❌ no payment rate; `power_kw` is a setpoint |
